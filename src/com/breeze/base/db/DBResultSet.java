...
/**
 * 这个类搞死我，因为要实现COMMDB的方便操作
 * 所以COMMDB只是一个查询接口，没有暴露connection
 * 这样导致DBCP连接池的链接总是没有释放
 * 所以使用代理模式，做了这个resultset，目的是为了在rs.close的时候
 * 同时关闭connection和statment
 * 因为害怕性能问题，所以这个类自己手动写的，没有使用java的代理反射机制
 * ResultSet要实现的方法太多，写死我了
 * @author Administrator
 *
 */
public class DBResultSet implements ResultSet {
	private Logger log = Logger.getLogger("com.breeze.base.db.DBResultSet");
	private ResultSet rs;
	private Statement st;
	private Connection con;
	
	DBResultSet(ResultSet r,Statement s,Connection c){
		this.rs = r;
		this.st = s;
		this.con = c;
	}
	...
	public void close() throws SQLException {
		log.fine("begin close resultset");
		this.rs.close();
		this.st.close();
		if(this.con.getAutoCommit() == true){
			...
			//2014-07-29罗光瑜修改，为做数据库事务而做的准备工作，如果是自动提交的就直接关闭
			this.con.close();
			log.fine("close one auto trans connection");
			return;
		}
		log.fine("connection is trans and will not be closed!");
		return;
	}

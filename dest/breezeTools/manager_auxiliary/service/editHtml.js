...
0.02 罗光瑜 2016-09-14解决跳转文件后?后字符无法解析问题
0.03 罗光瑜 配套新版本的模拟文件处理
    ...
    FW.register({
        ...
        */
        ...
        "public": {
            ...
            */
            "showPageFromUrl": function() {
                ...
                //if (传入的url不为空){获取文件信息
                if (fileUrl != null) {
					//2016-10-16日罗光瑜修改，先申请锁
					var lockRs = this.API.doServer("syncoper","file",{
						url:fileUrl,
						operType:"apply"
					});
					if (lockRs.code!=0){
						FW.alert("该文件其他人正在编辑,可以查看，但不能保存");
					}
					this.MY.lockUrl = fileUrl;
					this.MY.sync = ""+lockRs.data;
            ...
            *@function
            *@memberOf editHtml
            *@name public$showTopMenuButton
            *@description [功能]顶部的下拉按钮要被修改，支持多种模拟的情况
            *[思路]首先要判断当前是否是devxx的环境，即这个环境是否可以接受模拟数据，如果可以就使用多种方式 接受这个模拟数据
            */
            "showTopMenuButton": function() {
                this.API.show("topMenuButton", null, "topMenuButton");
            },
            /**
            *@function
            *@memberOf editHtml
            *@name public$showMoke
            *@description [功能]显示所有可预览的模拟数据
            *[思路]
            */
            "showMoke": function() {
                //到后台查询所有场景
                var jspUrl = "/" + this.MY.jspPath + '/' + this.MY.jspFileName;
                var url = jspUrl.replace(/[\\\/]+/ig, "_");
                var result = this.API.doServer("getMokeNames", "moke", {
                    url: url
                });
                ...
                this.MY.type = "moke";
                this.API.show("mokeMgr", data, "mainedit");
            },
            /**
            *@function
            *@memberOf editHtml
            *@name public$showOneMoke
            *@description [功能]根据输入的标识，显示具体的模拟数据信息
            *@param sig 模拟标识
            */
            "showOneMoke": function(sig) {
                //获取标url
                var jspUrl = "/" + this.MY.jspPath + '/' + this.MY.jspFileName;
                var url = jspUrl.replace(/[\\\/]+/ig, "_");
                var result = this.API.doServer("getOne", "moke", {
                    url: url,
                    sig: sig
                });
                if (result == null || result.code != 0 || result.data == null) {
                    FW.alert("未找到对应标识的模拟数据");
                    return;
                }
                //整理数据
                var data = result.data;
                var text = formatJS.js_beautify(FW.use().toJSONString(result.data));
                //显示到页面
                this.API.show("onemoke", {
                    obj: data,
                    text: text,
                    sig: sig
                },
        ...
        "private": {
            ...
            */
            "openCreator": function(creatorName, fileUrl) {
                //合成creator的url
                var url = "./" + creatorName + "Creator.jsp?fileUrl=" + encodeURIComponent(fileUrl.replace(/[#\?].*$/i, ""));
                //显示视图
            ...
            */
            "saveHTML": function() {
				//2016-10-17罗光瑜修改，保存前先申请锁
				var lockRs = this.API.doServer("syncoper","file",{
						operType:"apply",
						url:this.MY.lockUrl,
						lasttime:this.MY.sync
				});
				if (lockRs.code != 0){
					alert("其他用户正在编辑，请重新刷新获取新版本后再保存");
					return;
				}
                //读取html内容
                var code_panel = this.API.find("#infoForm")[0];
        ...
        "FireEvent": {
            ...
            *@function
            *@memberOf editHtml
            *@name FireEvent$goSceneView
            *@description [功能]去浏览定制的场景，带上模拟数据
            *[思路]
            */
            "goSceneView": function() {
                //获取数据
                var sceneinput = $("#sceneinput").val();
            ...
            *@function
            *@memberOf editHtml
            *@name FireEvent$showOneMoke
            *@description [功能]显示其中一个具体的模拟信息
            *[思路]显示其中一个具体的模拟信息
            *@param sig 模拟标识
            */
            "showOneMoke": function(sig) {
                //调用公有方法显示模拟标识
                this.showOneMoke(sig);
            },
            /**
            *@function
            *@memberOf editHtml
            *@name FireEvent$saveOneMoke
            *@description [功能]保存其中一个场景信息
            *[思路]这里描述实现的基本思路
            ...
            *@param sig 场景标识
            */
            "saveOneMoke": function(sig) {
                //获取标url
                var jspUrl = "/" + this.MY.jspPath + '/' + this.MY.jspFileName;
                var url = jspUrl.replace(/[\\\/]+/ig, "_");
                var text = $("#mokeText").val();
                var sigCtx = eval("(" + text + ")");

                var result = this.API.doServer("saveOne", "moke", {
                    url: url,
                    sig: sig,
                    content: sigCtx
                });
            ...
            *@function
            *@memberOf editHtml
            *@name FireEvent$previewOneMoke
            *@description [功能]点击后预览其中一个模拟数据场景
            *[思路]这里描述实现的基本思路
            *[接口.this.MY.xxx]这里描述内部全局变量定义
            *[接口.service.pkg.name.param]{这里描述服务pkg.name要传入参数内容}
            *[接口.service.pkg.name.return]{这里描述doserver返回值的json的结构}
            ...
            *@param qstr 参数
            */
            "previewOneMoke": function(sig, qstr) {
                //toDo
                var jspUrl = "../" + this.MY.jspPath + '/' + this.MY.jspFileName + "?mokeSignal=" + sig + "&" + qstr;
                window.open(jspUrl);
            },
            /**
            *@function
            *@memberOf editHtml
            *@name FireEvent$previewMoke
            *@description [功能]这是点击后打开模拟数据的预览列表，即模拟数据管理
            */
            "previewMoke": function() {
                //直接调用公有方法显示
                this.showMoke();
            },
            /**
            ...
            */
            "preview": function(scene) {
                //调用私有的getPreviewUrl获取预览地址
                var url = "../" + this.MY.jspPath + '/' + this.MY.jspFileName;;
                //用window.open打开新地址
                window.open(url);
            },
            /**
            *@function
            *@memberOf editHtml
            *@name FireEvent$newMokeMask
            *@description [功能]选择场景
            *[思路]选择场景的
            */
            "newMokeMask": function() {
                this.API.mask("sceneselect", null, "500");
            }
        }
    },
    module, '0.03');
